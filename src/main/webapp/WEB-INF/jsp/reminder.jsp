...
<table class="table table-hover">
	<thead><tr><th>ID</th><th>Edit</th><th>Delete</th><th>Name</th><th>Type</th><th>Frequency</th><th>Unit</th><th>Expires On</th><th>Renew</th><th>Email Reminder</th><th>SMS Reminder</th></tr></thead>
	<tbody>
		<c:forEach items="${reminderList}" var="reminder">
            <c:if test="${reminder.urgentCountDays<=0}">
                <tr style="background-color: #F65737;color:white">
            </c:if>
            <c:if test="${reminder.urgentCountDays>=0 and reminder.urgentCountDays<30}">
                <tr style="background-color: #39C809;color:white">
            </c:if>
            <c:if test="${reminder.urgentCountDays>=30 and reminder.urgentCountDays<60}">
                <tr style="background-color: #91A4FA;color:white">
            </c:if>
            <c:if test="${reminder.urgentCountDays>60}">
                <tr>
            </c:if>
			<td>${reminder.id}</td>
			...
			<td><a href="/reminder/renew?id=${reminder.id}" ><img src="${pageContext.request.contextPath}/images/renew.png" width="30"  height="30"/></a></td>
